...
document.getElementById("loginForm").addEventListener("submit", function (e) {
  ...
  // Lista de usuários permitidos (Simulando banco de dados)
  const usuarios = [
    { email: "admin@gmail.com", senha: "1234", nome: "Carmem Lúcia",    cargo: "proprietaria" },
    { email: "erika@gmail.com", senha: "1234", nome: "Erika Priscilla", cargo: "colaborador"  },
  ];

  const encontrado = usuarios.find(u => u.email === usuario && u.senha === senha);

  if (encontrado) {
    // Salva o nome e e-mail na memória do navegador para usar nas outras telas
    sessionStorage.setItem("usuarioNome",  encontrado.nome);
    sessionStorage.setItem("usuarioEmail", encontrado.email);
    sessionStorage.setItem("usuarioCargo", encontrado.cargo);
